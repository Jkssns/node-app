<template>
	<div class="big_container">
		<router-link to="/test">111</router-link>
		...
		<div class="container">
			...
			<ul style="display: flex;">
				<li 
					...
					style="margin: 20px;width: 50px;height: 50px;border: 1px solid red;"
					@click="aaa(item)"
				>{{ item.active }}</li>
			</ul>
		...
		const move = (moveEvent) => {
			...
			liRefs.value.forEach((item, index) => {
				...
				if (
					...
					|| domL > l && domL < r && domB > t && domT < b
					|| domL < l && domR > r && domB > b && domT < b
					|| domL < l && domR > r && domB > t && domT < t
					|| domL < l && domR > r && domB < b && domT > t
				) {
